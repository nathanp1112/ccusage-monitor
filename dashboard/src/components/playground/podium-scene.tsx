...
export interface PodiumLeader {
  rank: number
  name: string
  cost: string // pre-formatted like "$4,230"
}

interface PodiumSceneProps {
  leaders: PodiumLeader[] // expects exactly 3 entries, sorted by rank (1, 2, 3)
}
...
const RANK_CONFIG: Record<number, Omit<LeaderEntry, 'rank' | 'name' | 'cost'>> = {
  1: { color: '#ffd700', medalColor: '#ffd700', spotlightColor: '#ffd700', podiumHeight: 2.4, xPos: 0 },
  2: { color: '#c0c0c0', medalColor: '#c0c0c0', spotlightColor: '#e8e8e8', podiumHeight: 1.8, xPos: -2.6 },
  3: { color: '#cd7f32', medalColor: '#cd7f32', spotlightColor: '#cd7f32', podiumHeight: 1.4, xPos: 2.6 },
}
...
function Scene({ leaders }: { leaders: LeaderEntry[] }) {
  return (
    <>
...
// ---------------------------------------------------------------------------
// PodiumScene Component
// ---------------------------------------------------------------------------

export function PodiumScene({ leaders }: PodiumSceneProps) {
  const derivedLeaders = useMemo<LeaderEntry[]>(() => {
    return leaders.map((leader) => {
      const config = RANK_CONFIG[leader.rank]
      return {
        rank: leader.rank,
        name: leader.name,
        cost: leader.cost,
