...
type ReactionState = 'idle' | 'typing' | 'overloaded' | 'celebrating'
...
const BUTTON_CONFIG: { state: ReactionState; label: string; color: string; activeColor: string }[] =
  [
    {
      state: 'idle',
      label: 'Idle',
      color: 'bg-slate-700 hover:bg-slate-600',
      activeColor: 'bg-emerald-600 ring-2 ring-emerald-400',
    },
    {
      state: 'typing',
      label: 'Typing',
...
export default function MascotPage() {
  ...
  return (
    <div className="relative flex h-[calc(100vh-48px)] flex-col bg-slate-950">
      {/* Title overlay */}
      <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex flex-col items-center pt-6">
        <h1 className="text-2xl font-bold tracking-tight text-white/90 sm:text-3xl">
          Claude Mascot Reactions
        </h1>
        <p className="mt-1 text-sm text-white/40">
          Click a reaction to see the mascot respond
        </p>
      </div>
      ...
      {/* Button bar */}
      <div className="flex items-center justify-center gap-3 border-t border-white/10 bg-slate-900/90 px-4 py-4 backdrop-blur-md">
        {BUTTON_CONFIG.map((btn) => (
          <button
            key={btn.state}
            onClick={() => setState(btn.state)}
            className={`rounded-lg px-4 py-2 text-sm font-medium text-white transition-all duration-200 sm:px-6 sm:py-2.5 ${
              state === btn.state ? btn.activeColor : btn.color
            }`}
          >
            {btn.label}
          </button>
