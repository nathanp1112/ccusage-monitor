...
function Needle() {
  ...
  useFrame((_, delta) => {
    ...
    const currentValue = TARGET_VALUE * Math.min(dampedT, 1.0 + 0.001)
    ...
    if (arrived) {
      ...
      groupRef.current.rotation.z = valueToAngle(TARGET_VALUE) + wobble
    }
...
function DigitalReadout() {
  ...
  useFrame((_, delta) => {
    ...
    const t = timeRef.current
    const dampedT = 1 - Math.exp(-2.5 * t) * Math.cos(6 * t)
    const current = TARGET_VALUE * Math.min(dampedT, 1.0)
...
export default function MeterPage() {
  return (
    <div style={{ position: 'relative', width: '100%', height: 'calc(100vh - 48px)', background: '#0a0a0a' }}>
      {/* Title overlay */}
      <div
        style={{
          position: 'absolute',
      ...
      >
        <h1
          style={{
            ...
            margin: 0,
            textShadow: '0 0 20px rgba(255, 153, 68, 0.4)',
          }}
        >
          3D Usage Meter
        </h1>
