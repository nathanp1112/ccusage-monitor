...
const MEMBERS = [
  { name: 'Alice', cost: 4230 },
  { name: 'Bob', cost: 3150 },
  { name: 'Charlie', cost: 2890 },
  { name: 'Diana', cost: 2450 },
  { name: 'Eve', cost: 1980 },
  { name: 'Frank', cost: 1650 },
  { name: 'Grace', cost: 1200 },
  { name: 'Henry', cost: 890 },
]

// Color palette from cool blue to warm orange
const PALETTE = [
  '#1e40af', // deep blue
  '#2563eb', // blue
  '#0891b2', // cyan
...
const MAX_COST = Math.max(...MEMBERS.map((m) => m.cost))
const MIN_HEIGHT = 1
const MAX_HEIGHT = 8

function costToHeight(cost: number): number {
  return MIN_HEIGHT + ((cost - 890) / (MAX_COST - 890)) * (MAX_HEIGHT - MIN_HEIGHT)
}
  ...
  return (
    <div className="relative" style={{ height: 'calc(100vh - 48px)' }}>
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex flex-col items-center pt-6">
        ...
        >
          Team usage costs visualized as a city skyline
        </p>
      </div>
